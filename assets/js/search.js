...
var displayingAutcompleteResults = [];
    ...
    if (lastSearch != searchVal)
    {
      ...
      if (results.length > 0)
      {
        ...
        for (i=0; i < autoCompleteResultLimit && i < results.length; i++)
        {
          ...
          resultsOutput.push("<div class='autoCompleteResult pl-10 pr-10' id='autoCompleteResult" + i + "' onclick='loadPage(\"" + metadata.pages[results[i].topic].url + "\")'>");
